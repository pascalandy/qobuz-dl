...
from qobuz_dl.downloader import _get_description
...
def test_download_description_includes_quality_and_disc_when_present():
    item = {"bit_depth": 24, "sampling_rate": 96}

    assert _get_description(item, "Track Title") == "Track Title [24/96]"
    assert _get_description(item, "Track Title", multiple=2) == (
        "[Disc 2] Track Title [24/96]"
    )
